...
use manifest::{Manifest, task::TaskConfig};
use serde::de::DeserializeOwned;
use tracing::info;

use crate::task::{GetTask, Task};
...
/// └── ...
#[derive(Debug)]
pub struct Resource<Action: Debug + Clone> {
    pub root: PathBuf,
    pub manifest: Manifest,
    pub task_config: TaskConfig<Action>,
}

impl<ActionSet: Debug + Clone + DeserializeOwned> Load for Resource<ActionSet> {
...
impl<Action: Debug + Clone> GetTask<Action> for Resource<Action> {
...
impl<Action: Debug + Clone> GetTemplate for Resource<Action> {
    fn get_template(&self, path: impl AsRef<Path>) -> anyhow::Result<DynamicImage> {
...
impl<Action: Debug + Clone> ResRoot for Resource<Action> {
    fn res_root(&self) -> &Path {
        &self.root
    }
}
...
impl<T: Load> GitRepoResource<T> {
    pub fn try_init(
        ...
        download_repo_zip(&repo_url, parent_dir)?;
    ...
    pub fn try_load_or_init(
        ...
        let target_dir = target_dir.as_ref();
        let repo_url = repo_url.as_ref().to_string();
        let subpath = subpath.map(|s| s.to_string()).unwrap_or(String::new());
        if let Ok(resource) = T::load(&target_dir) {
            ...
            Self::try_init(&target_dir, &repo_url, Some(&subpath))?;
            let manifest = fs::read_to_string(target_dir.join("manifest.toml"))?;
            let manifest = toml::from_str(&manifest)?;
    ...
    pub fn update(self) -> Result<Self, anyhow::Error> {
        let manifest = fetch_manifest(&self.repo_url)?;
        ...
        Self::try_load_or_init(&self.root, self.repo_url, Some(&self.subpath))
    }
}

/// MARK: Functions

fn download_repo_zip(
    repo_url: impl AsRef<str>,
    dir: impl AsRef<Path>,
) -> Result<(), anyhow::Error> {
...
fn fetch_manifest(repo_url: impl AsRef<str>) -> Result<Manifest, anyhow::Error> {
    let manifest_bytes = fetch_file_from_github(repo_url, "manifest.toml")?;
...
mod test {
    ...
    fn test_update_resource() {
        ...
        .unwrap();
        let resource = resource.update().unwrap();
        println!("{:?}", resource.manifest);
    }

    fn test_fetch_manifest() {
        let version = fetch_manifest("https://github.com/AzurIce/aah-resources").unwrap();
        println!("{:?}", version);
    }
}
